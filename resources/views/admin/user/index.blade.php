            ...
            <div class="col-lg-12">
                <div class="tabelhed d-flex justify-content-between">
                    <div class="col-lg-2 col-md-2 col-sm-2 d-flex">
                        @can('user_create')
                        <a class="btn text-center btn-primary" href="{{ route('users.create') }}"> Add</a>
                        @endcan
                    </div>
                    ...
                    <div class="col-lg-10 col-md-10"> 
                        <div class="right-item d-flex justify-content-end" >
                            <form action="" method="GET" class="d-flex">
                                ...
                                <a href="{{ route('users.index') }}" class="btn-sm reload-btn">
                                ...
                                @if(count($data)>0)
                                    ...
                                    @foreach($data as $key => $value)
                                        <tr data-entry-id="{{ $value->id }}">
                                            ...
                                            <td>{{ $value->name ?? '' }}</td>
                                            <td>{{ $value->email ?? '' }}</td>
                                            <td>{{ $value->roles()->first()->title ?? '' }}</td>
                                            <td class="text-center">           
                                                @can('user_show')
                                                <a href="{{ route('users.show', $value->id) }}" class="btn btn-sm btn-icon p-1">
                                                    <i class="fe fe-eye mx-1" data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="top" title="View"></i>
                                                </a>
                                                @endcan
                                                @can('user_edit')
                                                <a href="{{ route('users.edit', $value->id) }}" class="btn btn-sm btn-icon p-1">
                                                ...
                                                <form action="{{ route('users.destroy', $value->id) }}" method="POST" style="display:inline;">
                                                    @csrf
                                                    @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-icon p-1 delete-record" onclick="return confirm('Are you sure you want to delete this user?')"><i class="fe fe-trash-2" data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="top" data-bs-html="true" title="Delete"></i></button> 
                                                </form>
                                                @endcan
                                            </td>
                                        </tr>
